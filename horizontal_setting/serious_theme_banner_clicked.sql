...
SELECT dt,variation_id,touchpoint,count(distinct host_id) as hosts
FROM (
SELECT DISTINCT
A.dt,
variation_id,
B.host_id,
CASE WHEN ca.user_id is not null and cb.option = 'isNoFrills' then 'both_tab_banner_clicked'
     WHEN ca.user_id is not null then 'gamesettings_tab_changed'
     WHEN cb.option = 'isNoFrills' then 'horizontal_serious_theme_clicked'
     ELSE 'none'
     END as touchpoint,
