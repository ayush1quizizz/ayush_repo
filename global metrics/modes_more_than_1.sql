...
SELECT variation_id,
       host_id,
       count(distinct game_type) as game_modes
FROM ( 
    SELECT distinct
       a.variation_id,
       g.host_id,
       g.game_id,
       case when g.game_type in ('tp','pres_tp') then 'tp' 
            when g.game_type in ('async','pres_async','flashcard_async') then 'async' 
            when g.game_type in ('test') then 'test' 
            when g.game_type in ('live') then 'live' 
            when g.game_type in ('team') then 'team'
            when g.game_type in ('mystic_peak','mastery_peak') then 'mystic_peak' 
       end as game_type
