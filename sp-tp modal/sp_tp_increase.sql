...
from clean.game g
where date(g.created_at) >= '2025-01-01'
and date(g.created_at) <= '2025-08-24'
...
where date(g.created_at) >= '2025-08-25'
...
and date(g.created_at) >= '2025-08-18'
and date(g.created_at) <= '2025-08-22'
and g.host_occupation = 'teacher'
and g.host_country = 'US'
and g.is_classroom_game
--where b.tp_games > 0
...
and date(g.created_at) <= '2025-09-05'
and g.host_occupation = 'teacher'
and g.host_country = 'US'
and g.is_classroom_game
--where b.tp_games > 0
...
,total_games_post as (
select count(distinct game_id) as post_total_games
from clean.game g
...
,total_games_pre as (
select count(distinct game_id) as pre_total_games
from clean.game g
where date(g.created_at) >= '2025-08-18'
and date(g.created_at) <= '2025-08-22'
and g.host_occupation = 'teacher'
and g.host_country = 'US'
and g.is_classroom_game
...
select count(distinct case when tp_status = 'tp_acquired' then host_id end) as tp_acquired_hosts,
       ...
       sum(case when time_period = 'pre_games' and tp_status = 'tp_acquired' then games end) as pre_games_tp_hosts_games,
       sum(case when time_period = 'pre_games' and tp_status = 'no_tp_acquired' then games end) as pre_games_no_tp_hosts_games,
       sum(case when time_period = 'post_games' and tp_status = 'tp_acquired' then games end) as post_games_tp_hosts_games,
       sum(case when time_period = 'post_games' and tp_status = 'no_tp_acquired' then games end) as post_games_no_tp_hosts_games,
from (
...
cross join total_games_pre b
cross join total_games_post c
