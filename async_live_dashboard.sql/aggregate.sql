...
where date(experiment_date) between '2025-08-22' and '2025-08-27'
...
count(distinct case when tb.total_time_in_min >= 2 and g.is_classroom_game then tb.game_id end) as time_spent_more_than_4_min
from base a
...
on st.user_id = g.host_id
and st.quiz_id = g.quiz_id
and date(st.created_at) = date(g.created_at)
